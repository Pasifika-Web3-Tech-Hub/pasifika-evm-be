...
contract ProfitSharingTest is Test {
    ...
    function testProfitClaiming() public {
        ...
        assertTrue(membership.isEligibleForProfitSharing(member1, 1970), "Member1 should be eligible for profit sharing");
        assertTrue(membership.isEligibleForProfitSharing(member2, 1970), "Member2 should be eligible for profit sharing");
        assertFalse(membership.isEligibleForProfitSharing(member3, 1970), "Member3 should not be eligible for profit sharing");
    ...
    function testProfitSharingStateWithoutTransfers() public {
        ...
        if (initialState) {
            ...
            membership.initiateProfitSharing();
            assertTrue(membership.profitSharingInProgress(), "Profit sharing should be in progress after initiation");
            assertGt(membership.currentProfitSharingYear(), 0, "Current profit sharing year should be non-zero after initiation");
        }
