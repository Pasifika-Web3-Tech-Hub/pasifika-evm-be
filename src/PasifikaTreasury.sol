...
contract PasifikaTreasury is AccessControl, ReentrancyGuard, Pausable {
     ...
     */
    function withdrawFunds(address recipient, uint256 amount) external nonReentrant whenNotPaused onlyRole(SPENDER_ROLE) returns (bool) {
        require(recipient != address(0), "PasifikaTreasury: zero address");
        require(amount > 0, "PasifikaTreasury: zero amount");
        ...
        if (funds[UNALLOCATED_FUND].balance >= amount) {
            ...
            if (remainingAmount > 0) {
                require(funds[UNALLOCATED_FUND].balance >= remainingAmount, "PasifikaTreasury: insufficient funds after allocation");
     ...
     */
    function updateAllFundAllocations(
        bytes32[] memory updatedFundNames,
        uint256[] memory updatedAllocations
     ...
     */
    function getFundDetails(bytes32 fundName) external view returns (
        string memory name,
        uint256 allocation,
        uint256 balance,
        bool active
