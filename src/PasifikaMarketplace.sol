...
contract PasifikaMarketplace is AccessControl, ReentrancyGuard, Pausable {
    ...
    function purchaseItem(uint256 listingId) external payable whenNotPaused nonReentrant {
        ...
        try IERC2981(listing.nftContract).royaltyInfo(listing.tokenId, finalPrice) returns (
            address receiver, uint256 royaltyFee
        ) {
    ...
    function cancelListing(uint256 listingId) external nonReentrant {
        ...
        require(
            msg.sender == listing.seller || hasRole(MARKET_MANAGER_ROLE, msg.sender),
