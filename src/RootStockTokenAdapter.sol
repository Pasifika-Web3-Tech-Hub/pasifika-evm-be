...
contract RootStockTokenAdapter is PasifikaTokenAdapter, AccessControl {
     ...
     */
    function transferToken(address token, address recipient, uint256 amount) external override returns (bool success) {
        require(supportedTokens[token], "RootStockTokenAdapter: token not supported");
        ...
        (bool callSuccess, bytes memory data) = token.call(
            abi.encodeWithSelector(
                bytes4(keccak256("transferFrom(address,address,uint256)")), msg.sender, recipient, amount
            )
        );

        require(
            callSuccess && (data.length == 0 || abi.decode(data, (bool))),
