...
contract PasifikaMoneyTransfer is AccessControl, Pausable, ReentrancyGuard {
    ...
    // Transaction types
    enum TransactionType { Direct, Scheduled, Conditional }
    
    // Transaction status
    enum TransactionStatus { Pending, Completed, Cancelled, Disputed }
    
    // Condition types for conditional transfers
    enum ConditionType { None, TimeDelay, MultiSig, DocumentVerification, ExternalOracle }
    ...
    // Events
    event TransactionCreated(uint256 indexed transactionId, address indexed sender, address indexed recipient, uint256 amount, TransactionType transactionType);
    event Transfer(address indexed from, address indexed to, uint256 amount, string message);
    event TransactionCompleted(uint256 indexed id, address indexed sender, address indexed receiver, uint256 amount, uint256 fee);
    ...
    event ScheduledTransferCreated(uint256 indexed transferId, address indexed sender, address indexed recipient, uint256 amount, uint256 interval, uint256 repetitions);
    event ScheduledTransferUpdated(uint256 indexed transferId, bool active);
    event ScheduledTransferExecuted(uint256 indexed transferId, address indexed sender, address indexed recipient, uint256 amount);
    event ScheduledTransferCancelled(uint256 indexed transferId);
    event CommunityCollectionCreated(uint256 indexed collectionId, address indexed creator, string purpose, uint256 goal, uint256 deadline);
     ...
     */
    function transfer(address recipient, uint256 amount, string memory memo) external payable whenNotPaused nonReentrant returns (uint256) {
     ...
     */
    function createScheduledTransfer(address recipient, uint256 amount, uint256 interval, uint256 repetitions, string memory memo) external payable whenNotPaused nonReentrant returns (uint256) {
        require(recipient != address(0), "PasifikaMoneyTransfer: invalid recipient");
        require(recipient != msg.sender, "PasifikaMoneyTransfer: cannot send to self");
        require(amount > 0, "PasifikaMoneyTransfer: amount must be positive");
    ...
    function executeScheduledTransfer(uint256 scheduledTransferId) external whenNotPaused nonReentrant {
        ...
        require(block.timestamp >= scheduledTransfer.nextExecutionTime, "PasifikaMoneyTransfer: not ready for execution");
     ...
     */
    function createCommunityCollection(string memory purpose, uint256 goal, uint256 deadline) external whenNotPaused nonReentrant returns (uint256) {
     ...
     */
    function payoutFromCollection(uint256 collectionId, address recipient, uint256 amount) external whenNotPaused nonReentrant onlyRole(ADMIN_ROLE) {
        require(recipient != address(0), "PasifikaMoneyTransfer: invalid recipient");
        require(amount > 0, "PasifikaMoneyTransfer: amount must be positive");
        require(collectionId < current(_communityCollectionIdCounter), "PasifikaMoneyTransfer: invalid collection ID");
     ...
     */
    function setFeeParameters(uint256 _baseFeePercent, uint256 _minFee, uint256 _maxFee) external onlyRole(FEE_MANAGER_ROLE) {
     ...
     */
    function getUserTransactions(address user, uint256 offset, uint256 limit) external view returns (uint256[] memory) {
     ...
     */
    function getCommunityCollection(uint256 collectionId) external view returns (
        string memory name,
        address creator,
        uint256 goal,
        uint256 raised,
